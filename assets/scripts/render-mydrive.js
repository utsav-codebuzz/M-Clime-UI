...
function attachDeleteModalEvents(fileId) {
  ...
  if (deleteBtn) {
    deleteBtn.addEventListener("click", function () {
      ...
      const fileIndex = window.mydriveFilesData.findIndex(
        (f) => f.id == fileId
      );
      if (fileIndex !== -1) {
        window.mydriveFilesData.splice(fileIndex, 1);
...
function applyFilters(filterType, value, customData = null) {
  ...
  if (!window.originalFilesData) {
    window.originalFilesData = [...window.mydriveFilesData];
  }
  ...
  window.mydriveFilesData = filteredFiles;
