    ...
    setupGlobalRestoreHandler() {
        if (!this.globalHandlerSetup) {
            this.globalClickHandler = (e) => {
                const menuItem = e.target.closest('.file-menu li[data-action="restore"]');
                if (menuItem) {
                    const isBinPage = window.location.pathname.includes('bin.html') || 
                                     document.querySelector('.page-title')?.textContent.trim() === 'Bin' ||
        ...
        if (window.binFilesData && Array.isArray(window.binFilesData)) {
            ...
            if (fileIndex !== -1) {
                ...
                if (window.originalFilesData) {
                    const originalIndex = window.originalFilesData.findIndex((f) => f.id == fileId);
                    if (originalIndex !== -1) {
                ...
                if (currentView === 'grid' && typeof renderGridViewStarred === 'function') {
                    renderGridViewStarred();
                } else if (currentView === 'list' && typeof renderListViewStarred === 'function') {
